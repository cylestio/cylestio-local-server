...
# Using relative imports since we're running from src directory
from .models.base import init_db, get_db, create_all
from .models.event import Event
from .models.llm_interaction import LLMInteraction
from .models.llm_attribute import LLMAttribute
...
llm_attributes_count = db_session.query(func.count(LLMAttribute.id)).scalar()
...
spans_count = db_session.query(func.count(Span.id)).scalar()

print(f"Events: {events_count}")
print(f"LLM Interactions: {llm_interactions_count}")
print(f"LLM Attributes: {llm_attributes_count}")
...
if llm_interactions_count > 0:
    ...
    # LLM Attributes for this interaction
    attr_sample = db_session.query(LLMAttribute).filter_by(llm_interaction_id=llm_sample.id).limit(5).all()
    print(f"  Attributes count: {len(attr_sample)}")
    for attr in attr_sample:
        print(f"    {attr.key}: {attr.value[:50]}..." if len(attr.value or '') > 50 else f"    {attr.key}: {attr.value}")
...
if framework_events_count > 0:
    ...
    print(f"  Framework: {framework_sample.framework}")
